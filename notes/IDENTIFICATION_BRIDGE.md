...
In-paper anchor: `paper/MERSENNE_PRIMES_PREPRINT.md` (bridge and margin interface).

## Goal
Identify the canonical endpoint representative with the ordinary problem-side target for `prove infinitude of Mersenne primes by routing admissible exponential-prime states through coercive exponential response, Lucas capture, compactness, rigidity, residue transfer, and strict coherence`.
...
- endpoint lock: `residue_transfer`.
...
2. the endpoint-transfer lock `residue_transfer` identifies the surviving representative;
3. the coherence remainder `eps_coh` is not smuggled into the endpoint;
4. the strict closure margin `M_MP` remains positive.
...
Payload: verify `residue_transfer` against the constants registry and runtime certificate.
...
If lock persistence, determining-class uniqueness, and strict coherence hold, then the endpoint representative is identified and the final margin `M_MP` is meaningful.

## Current Instantiation

- endpoint-transfer artifact key: `residue_transfer`
