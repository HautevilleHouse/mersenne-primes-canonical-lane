...
In-paper anchor: `paper/MERSENNE_PRIMES_PREPRINT.md` (`MP_G3`).

## Goal
Make the compactness/no-Zeno gate explicit for `prove infinitude of Mersenne primes by routing admissible exponential-prime states through coercive exponential response, Lucas capture, compactness, rigidity, residue transfer, and strict coherence`: near-failure behavior must be captured, not allowed to drift or accumulate invisibly.
...
`MP_G3` closes when `kappa_compact` provides compactness/no-collapse control: normalized near-failure families are precompact and exponent windows do not collapse.
This is the first-failure capture contribution to `M_MP`.
...
If near-failure extraction and no-Zeno spacing hold, then `MP_G3` prevents hidden drift before rigidity is applied.
