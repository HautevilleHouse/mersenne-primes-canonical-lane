#!/usr/bin/env python3
"""Canonical-lane closure guard for the mersenne primes workspace."""
...
def compute_report(data: dict[str, Any], sigma_star: float | None, strict_coh_zero: bool) -> dict[str, Any]:
    ...
    native_names = ['MP_G1', 'MP_G2', 'MP_G3', 'MP_G4', 'MP_G5']
    gates = {name: ('PASS' if passed else 'FAIL') for name, passed in zip(native_names, gate_pass)}
    gates['MP_G6'] = 'PASS' if coh_pass else 'FAIL'
    gates['MP_GM'] = 'PASS' if gm_pass else 'FAIL'
    ...
    blockers['MP_G6'] = [] if coh_pass else ['eps_coh missing/invalid or strict-zero target failed']
    blockers['MP_GM'] = [] if gm_pass else ['strict margin <= 0 or upstream gates failed']
